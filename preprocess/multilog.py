...
from preprocess.apilog import ApiLog
import numpy as np
...
class MultiLog:
    ...
    def __init__(self, path):
        self.path = path
        self.file_list = list(self.path.glob("*.csv"))  # get all the csv files
        self.file_num = len(self.file_list)
        self.log_type = self.get_log_type()  # get the log type
        self.df_list = self.get_df_list()  # get all the dataframes
        self.tf_dic_list, self.tf_dic = self.get_api_tf_dic()  # get the tf_dic_list and tf_dic
        self.api_set = set(self.tf_dic.keys())
        self.df_list = self.add_tfidf()  # add tfidf column to the df_list
    ...
    def get_log_type(self):
        ...
        :return:
        """
        log_type = Log(self.file_list[0]).log_type
        for file in self.file_list:
            if Log(file).log_type != log_type:
                raise ValueError("The log files are not the same type.")
        print("The log type is {}".format(log_type))
        return log_type
